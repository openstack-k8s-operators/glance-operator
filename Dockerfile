...
ARG REMOTE_SOURCE=.
ARG REMOTE_SOURCE_DIR=glance-operator
...
RUN cp config/crd/bases/* $DEST_ROOT/bundle/
...
LABEL   com.redhat.component="glance-operator-container" \
        name="cn-osp/glance-operator" \
        version="0.0.1" \
        summary="glance Operator" \
        io.k8s.name="glance-operator" \
        io.k8s.description="This image includes the glance-operator"

ENV USER_UID=1001 \
    OPERATOR_TEMPLATES=/usr/share/glance-operator/templates/ \
    OPERATOR_BUNDLE=/usr/share/glance-operator/bundle/
