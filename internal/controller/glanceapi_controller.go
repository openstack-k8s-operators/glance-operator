...
import (
	...
	"github.com/openstack-k8s-operators/glance-operator/internal/glanceapi"
	...
	memcachedFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		for _, cr := range glanceAPIs.Items {
			if o.GetName() == cr.Spec.MemcachedInstance {
				...
				Log.Info(fmt.Sprintf("Memcached %s is used by GlanceAPI CR %s", o.GetName(), cr.Name))
				result = append(result, reconcile.Request{NamespacedName: name})
			}
		}
		if len(result) > 0 {
...
func (r *GlanceAPIReconciler) reconcileNormal(
	...
	for i := range availableBackends {
		...
		case "cinder":
			...
			if err != nil {
				...
				Log.Info("Error listing Cinder resources")
				instance.Status.Conditions.MarkFalse(
	...
	// Gate on acGuardReady: until the StatefulSet has rolled out the current
	// config, pods may still reference an intermediate secret that is in
	// neither the status nor current keep set, so pruning it would revoke an
	// application-credential still in use. Once ready, status has advanced to
	// current and the intermediate secrets are safe to prune.
	if acGuardReady {
...
func (r *GlanceAPIReconciler) createHashOfBackendConfig(
	...
	if hashMap, changed = util.SetHash(instance.Status.Hash, "backendHash", hash); changed {
		instance.Status.Hash = hashMap
		Log.Info(fmt.Sprintf("Backend hash %s - %s", "backendHash", hash))
	}
	return hash, changed, nil
}
...
func (r *GlanceAPIReconciler) ensureKeystoneEndpoints(
	...
	if len(instance.Annotations) == 0 ||
		...
		if err != nil {
			Log.Info(fmt.Sprintf("Endpoint %s not found", instance.Name))
			return glance.ResultRequeue, nil
		}
		return ctrlResult, nil
	}
...
func (r *GlanceAPIReconciler) getEndpointID(
	...
	if err != nil {
		if k8s_errors.IsNotFound(err) {
			...
			Log.Info(fmt.Sprintf("EndpointID not found for glanceAPI %s", instance.Name))
		}
		return epID, nil
	}
...
func (r *GlanceAPIReconciler) glanceAPIRefresh(
	...
	if err != nil {
		if k8s_errors.IsNotFound(err) {
			...
			Log.Info(fmt.Sprintf("GlanceAPI %s-api: Statefulset not found.", instance.Name))
			return nil
		}
		return err
	}
