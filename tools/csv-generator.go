...
import (
	...
	"github.com/openstack-k8s-operators/glance-operator/tools/helper"
...
var (
	...
	operatorImage = flag.String("operator-image-name", "quay.io/openstack-k8s-operators/glance-operator:devel", "optional")
)
...
//NewClusterServiceVersionData - Data arguments used to create glance operators's CSV manifest
...
func createOperatorDeployment(repo, namespace, deployClusterResources, operatorImage, tag, verbosity, pullPolicy string) *appsv1.Deployment {
	deployment := helper.CreateOperatorDeployment("glance-operator", namespace, "name", "glance-operator", "glance-operator", int32(1))
	container := helper.CreateOperatorContainer("glance-operator", operatorImage, verbosity, corev1.PullPolicy(pullPolicy))
...
func createClusterServiceVersion(data *NewClusterServiceVersionData) (*csvv1.ClusterServiceVersion, error) {
	...
	description := `
Install and configure OpenStack Glance.
`
	...
	strategySpec := csvv1.StrategyDetailsDeployment{
		Permissions: []csvv1.StrategyDeploymentPermissions{
			{
				ServiceAccountName: "glance-operator",
				Rules:              *rules,
			},
			{
	...
	return &csvv1.ClusterServiceVersion{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      "glance-operator." + data.CsvVersion,
			Namespace: data.Namespace,
			Annotations: map[string]string{

				"capabilities": "Basic Install",
				"categories":   "Image",
				"description":  "Creates and maintains Glance API deployments.",
			},
		},
		...
		Spec: csvv1.ClusterServiceVersionSpec{
			...
			Keywords:    []string{"Glance Operator", "OpenStack", "Glance", "Image"},
			...
			Provider: csvv1.AppLink{
				Name: "OpenStack K8s Operators Glance Operator project",
			},
			Links: []csvv1.AppLink{
				{
					Name: "Glance Operator",
					URL:  "https://github.com/openstack-k8s-operators/glance-operator/blob/master/README.md",
				},
				{
					Name: "Source Code",
			...
			Labels: map[string]string{
				"alm-owner-glance-operator": "glance-operator",
				"operated-by":               "glance-operator",
			},
				...
				Owned: []csvv1.CRDDescription{
					{
						...
						Version:     "v1beta1",
						Kind:        "GlanceAPI",
						DisplayName: "GlanceAPI",
						Description: "Glance API Instance",
					},
				},
			},
		},
